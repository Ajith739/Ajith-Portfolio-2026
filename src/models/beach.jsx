...
export function Beach(props) {
  ...
  const flagMaterial = React.useMemo(() => {
    ...
    mat.onBeforeCompile = (shader) => {
      ...
      shader.vertexShader = shader.vertexShader.replace(
        ...
        `#include <begin_vertex>
         // Simple flag waving based on local X axis
         float wave = sin(position.x * 5.0 + uTime * 4.0) * 0.1 * position.x;
         transformed.z += wave;
        `
      );
    };
  ...
  const treeMaterial = React.useMemo(() => {
    ...
    mat.onBeforeCompile = (shader) => {
      ...
      shader.vertexShader = shader.vertexShader.replace(
        '#include <begin_vertex>',
        `#include <begin_vertex>
         vec4 wPosWind = modelMatrix * vec4(position, 1.0);
         // Displace only vertices higher up (y > 4.0) in world coordinates
         float swayFactor = smoothstep(4.0, 15.0, wPosWind.y);
         transformed.x += sin(uTime * 1.5 + wPosWind.z * 0.5) * 0.3 * swayFactor;
         transformed.z += cos(uTime * 1.3 + wPosWind.x * 0.5) * 0.2 * swayFactor;
        `
      );
    };
    return mat;
  ...
  useFrame((state) => {
    ...
    if (treeMaterial.userData.shader) treeMaterial.userData.shader.uniforms.uTime.value = t
  })
